...
for i=1:NPTS
    ...
    for j=1:NPOSES
        ...
        r(row:row+1,1) = points_image_noisy(1:2,i,j) - h_est(1:2);% e=o-P(p)
...
g = J'*W*r; % g=JTe
mu = 0.01*max(damper,[],'all'); % mu:=tau*max(DTD)
...
for iter=1:NUM_ITERATIONS
    ...
    %H = J'*W*J+mu*damper;
    H = J'*W*J+mu*damper; % Using Schur elimination to solve (JTJ+uDTD)delta_p=g
    dx = H\g;
    ...
    if (rho>0)
        ...
        cam_pose_estimates = cam_pose_estimates_new;%%% p:=p_new
