...
for j=1:NPOSES
    ...
    % camera vector
    h = quiver3(cPo(1),cPo(2),cPo(3),zcam(1)*0.5,zcam(2)*0.5,zcam(3)*0.5,'k');
    set(h,'linewidth',2);
    h = quiver3(cPo(1),cPo(2),cPo(3),xcam(1)*0.5,xcam(2)*0.5,xcam(3)*0.5,'k');
    set(h,'linewidth',2);
    h = quiver3(cPo(1),cPo(2),cPo(3),ycam(1)*0.5,ycam(2)*0.5,ycam(3)*0.5,'k');
    set(h,'linewidth',2);
end
...
for i=1:NPTS
    ...
    for j=1:NPOSES
        ...
        r_init(row:row+1,1) = points_image_noisy(1:2,i,j)-h_est(1:2);
    end
end
...
err_prev = 10000;
v = 2;
cam_pose_estimates_temp = cam_pose_estimates;
t_start=cputime;
for iter=1:NUM_ITERATIONS
    ...
    for j=START_POSE:NPOSES
        ...
        cam_pose_estimates_temp(:,:,j) = update * cam_pose_estimates(:,:,j);
    end

    for i=1:NPTS
        p_world = points_world_estimate_temp(:,:,i);
        for j=1:NPOSES
            H_cam = cam_pose_estimates_temp(:,:,j);
            p_cam = H_cam * [p_world; 1];
            p_cam = p_cam(1:3);
            zc = p_cam(3);
            h_est = p_cam/zc;
    ...
    if (err < err_prev)
        ...
        cam_pose_estimates = cam_pose_estimates_temp;
...
for j=1:NPOSES
    ...
    % camera vector
    h = quiver3(cPo_new(1),cPo_new(2),cPo_new(3),zcam_new(1)*0.5,zcam_new(2)*0.5,zcam_new(3)*0.5,'m');
    set(h,'linewidth',2);
    h = quiver3(cPo_new(1),cPo_new(2),cPo_new(3),xcam_new(1)*0.5,xcam_new(2)*0.5,xcam_new(3)*0.5,'m');
    set(h,'linewidth',2);
    h = quiver3(cPo_new(1),cPo_new(2),cPo_new(3),ycam_new(1)*0.5,ycam_new(2)*0.5,ycam_new(3)*0.5,'m');
    set(h,'linewidth',2);
end
