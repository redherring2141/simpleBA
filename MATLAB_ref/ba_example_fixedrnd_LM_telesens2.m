...
t_start=cputime;
RANDOM_SAVE = 1; % 1 for random variable gen & save, 0 for random variable fixe & load
...
points_world_estimate_new = points_world_estimate;
...
for iter=1:NUM_ITERATIONS
    ...
    W = 1 ./ (1 + r2/sigsqrd);
    W = diag(W);
    damper = diag(diag(J'*W*J)); % DTD:=diag(JTJ)
    g = J'*W*r; % g=JTe
    if (iter==1)
        mu = 0.001*max(damper,[],'all'); % mu:=tau*max(DTD)    
    end
    ...
    %rho = (r'*r-r_new'*r_new) / (dx'*(mu*damper*dx+g));
    %rho = (norm(r2)^2-norm(r2_new)^2) / (dx'*(mu*damper*dx+g)); % rho := (|e|^2-|o-P(p_new)|^2)/(delta_pT(mu*delta_p+g))
    %rho = (norm(r)^2-norm(r_new)^2) / (dx'*(mu*damper*dx+g)); % rho := (|e|^2-|o-P(p_new)|^2)/(delta_pT(mu*delta_p+g))
    %rho = (sum(r2)-sum(r2_new)) / (dx'*(mu*damper*dx+g));
    %rho = (mean(r2)-mean(r2_new)) / (dx'*(mu*damper*dx+g));    
    rho = (r'*r-r_new'*r_new);
    ...
    if (rho>0)
        ...
        mu = mu*max(1/3, 1-(2*rho-1)^3); % mu:=mu*max(1/3,1-(2rho-1)^3)
        v = 2; % v:=2        
    else
        mu = mu*v; % mu:=mu*v
        v = v*2; % v:=2*v
